...
class GetAuditEventsUseCase:
    ...
    async def execute(
        ...
        async with self.uow:
            ...
            # Get audit events from MongoDB with pagination
            events, next_cursor = await self.audit_service.get_by_tenant_paginated(
            ...
            for event in events:
                ...
                if event_user_id:
                    user = await self.uow.users.get_by_id(UUID(event_user_id))
                    if user:
