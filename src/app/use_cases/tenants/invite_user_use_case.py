...
class InviteUserUseCase:
    ...
    - Creates audit event for compliance tracking (MongoDB)
    """
    ...
    async def execute(
        ...
        async with self.uow:
            ...
            # Get inviter's user info for name
            inviter_user = await self.uow.users.get_by_id(inviter_user_id)
            inviter_name = inviter_user.email if inviter_user else "Unknown"
            ...
            # Log audit event to MongoDB (after commit, best effort)
            ...
            # Return success response with Pydantic DTO (full invitation object)
            return Return.ok(
                InviteUserResponse(
