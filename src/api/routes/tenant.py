...
from src.depends import get_audit_service, get_current_user, get_unit_of_work
...
async def switch_tenant(
    ...
    # Execute use case
    use_case = SwitchTenantUseCase(uow, audit_service)
...
async def list_members(
    ...
    Raises:
        - 400 Bad Request: Invalid tenant_id format
        - 401 Unauthorized: Invalid or expired JWT
        - 403 Forbidden: INSUFFICIENT_ROLE or NOT_A_MEMBER
        - 500 Internal Server Error: Server error
    """
    ...
    # Execute use case
    use_case = ListMembersUseCase(uow)
    result = await use_case.execute(tenant_uuid, requester_user_id)
    ...
    if result.is_err():
        ...
        elif error.code in ("INSUFFICIENT_ROLE", "NOT_A_MEMBER"):
            raise ClientError(error, status_code=status.HTTP_403_FORBIDDEN)
        raise ServerError(error)
...
async def list_invitations(
    ...
    Retrieves all invitations for a tenant (pending, accepted, expired).
    Requires admin or owner permissions.
    ...
    # Execute use case
    use_case = ListInvitationsUseCase(uow)
    result = await use_case.execute(tenant_uuid, requester_user_id)
...
async def invite_user(
    ...
    # Execute use case
    use_case = InviteUserUseCase(uow, audit_service)
...
async def change_member_role(
    ...
    # Execute use case
    use_case = ChangeRoleUseCase(uow, audit_service)
    result = await use_case.execute(
...
async def remove_member(
    ...
    # Execute use case
    use_case = RemoveMemberUseCase(uow, audit_service)
...
async def delete_tenant(
    ...
    # Execute use case
    use_case = DeleteTenantUseCase(uow, audit_service)
