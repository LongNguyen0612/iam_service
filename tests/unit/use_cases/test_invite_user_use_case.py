...
@pytest.mark.asyncio
async def test_successful_invite_by_owner(mock_uow, mock_audit_service):
    ...
    # Inviter user for name lookup
    inviter_user = User(id=inviter_user_id, email="owner@company.com", password_hash="hash")
    mock_uow.users.get_by_id.return_value = inviter_user
    ...
    use_case = InviteUserUseCase(mock_uow, mock_audit_service)
    ...
    # Note: id will be newly generated UUID, not the mocked one
    assert response.id is not None
    assert len(response.id) == 36  # UUID string length
...
@pytest.mark.asyncio
async def test_successful_invite_by_admin(mock_uow, mock_audit_service):
    ...
    # Inviter user for name lookup
    inviter_user = User(id=inviter_user_id, email="admin@company.com", password_hash="hash")
    mock_uow.users.get_by_id.return_value = inviter_user
...
@pytest.mark.asyncio
async def test_insufficient_role_member(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_insufficient_role_viewer(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_duplicate_pending_invitation(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_user_already_member(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_invalid_role(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_inviter_not_a_member(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_invite_with_owner_role(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_invite_user_not_yet_registered(mock_uow, mock_audit_service):
