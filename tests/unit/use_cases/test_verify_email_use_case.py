...
@pytest.mark.asyncio
async def test_successful_email_verification(mock_uow, mock_audit_service):
    ...
    mock_user = User(
        ...
        email_verification_expires_at=datetime.now(UTC) + timedelta(hours=23)
    )
    mock_uow.users.get_by_verification_token.return_value = mock_user

    use_case = VerifyEmailUseCase(mock_uow, mock_audit_service)
...
@pytest.mark.asyncio
async def test_invalid_verification_token(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_expired_verification_token(mock_uow, mock_audit_service):
    ...
    mock_user = User(
        ...
        email_verified=False,
        email_verification_token=token,
        email_verification_expires_at=datetime.now(UTC) - timedelta(hours=25)  # Expired
    )
    mock_uow.users.get_by_verification_token.return_value = mock_user
...
@pytest.mark.asyncio
async def test_already_verified_user(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_missing_expiry_date(mock_uow, mock_audit_service):
...
@pytest.mark.asyncio
async def test_verification_token_at_boundary(mock_uow, mock_audit_service):
