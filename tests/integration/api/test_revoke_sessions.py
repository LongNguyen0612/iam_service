...
async def test_revoke_all_sessions_self(client: AsyncClient, db_session):
    ...
    db_session.add(user)
    await db_session.flush()  # Flush to ensure user exists before creating sessions

    tenant = Tenant(id=uuid4(), name="Test Tenant")
    db_session.add(tenant)
    await db_session.flush()  # Flush to ensure tenant exists
    ...
    await db_session.flush()  # Flush to ensure membership exists
