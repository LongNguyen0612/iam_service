...
async def test_admin_views_audit_log(client: AsyncClient):
    ...
    if len(data["events"]) > 1:
        ...
        ts1 = data["events"][0]["timestamp"]
        ts2 = data["events"][1]["timestamp"]
        first_timestamp = datetime.fromisoformat(ts1.replace("Z", "+00:00") if ts1.endswith("Z") else ts1)
        second_timestamp = datetime.fromisoformat(ts2.replace("Z", "+00:00") if ts2.endswith("Z") else ts2)
