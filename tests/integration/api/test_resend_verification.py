...
async def create_test_user_for_resend(
    ...
    user = User(
        ...
        email_verification_expires_at=(datetime.now(UTC) + timedelta(hours=23)) if with_token and not verified else None
    )
    db_session.add(user)
    await db_session.flush()
...
async def test_resend_extends_expiry(client: AsyncClient, db_session: AsyncSession):
    ...
    time_until_expiry = new_expiry - datetime.now(UTC)
